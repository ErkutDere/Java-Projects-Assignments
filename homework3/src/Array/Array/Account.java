...
public class Account {
    ...
    private int numBlockedAccounts;
    ...
    public void viewPosts(Account account) {
        int blockFlag = 0;
        for (int i = 0; i < account.numBlockedAccounts; ++i) {
            if (account.blockedAccounts[i].accountId == this.accountId) {
                blockFlag = 1;
            }
        }
        if (blockFlag == 0) {
    ...
    public void viewProfile(Account a) {
        int blockFlag = 0;
        for (int i = 0; i < a.numBlockedAccounts; ++i) {
            if (a.blockedAccounts[i].accountId == this.accountId) {
                blockFlag = 1;
            }
        }
        if (blockFlag == 0) {
    ...
    public void addMessageToInbox(Account a, Message m1) {
        int blockFlag = 0;
        for (int i = 0; i < this.numBlockedAccounts; i++) {
            if (this.blockedAccounts[i].accountId == a.accountId) {
                blockFlag = 1;
            }
        }
        boolean isFollower = false;
        for (int i = 0; i < this.numFollowers; i++) {
            if (this.followers[i].accountId == a.accountId) {
                isFollower = true;
                break;
            }
        }
    ...
    public void block(Account account) {
        blockedAccounts[numBlockedAccounts++] = account;
    }
}